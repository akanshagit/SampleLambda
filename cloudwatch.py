...
def lambda_handler(context, event):
    ...
    for region in regions:
        unsorted_tagged_arn_list.append(iterate_regions(region,key,value,logger))
    ...
    return create_dashboard(json.dumps(widget_json))

#LAMBDA HANDLER RETURN

def iterate_regions(region,key,value,logger):
    region_arn_list = []
    ...
    try: #Cross-Account Resources
        ...
        trusted_resources = get_trusted_roles(account_id['Account'])
...
def get_trusted_roles(account_id):
    request = boto3.client('iam')
    policy_arn = 'arn:aws:iam::' + account_id + ':policy/CrossAccountDashboardDiscoveryPolicy'
...
def create_dashboard(dashboard_body):
    cloudwatch = boto3.client('cloudwatch')
    response = cloudwatch.put_dashboard(DashboardName='Event-Dashboard', DashboardBody=dashboard_body)
    return response
